...
def sampleQ_phi(h_enc,share=None):
  with tf.variable_scope("sampleQ_phi",reuse=share):
    mu, log_sigma = tf.split(linear(h_enc, z_dim*2), 2, 1)  # diagonal stdev values, splits into 2 parts on axis=1
...
def transition(h):
  ...
  with tf.variable_scope("trans"):
    ...
    with tf.variable_scope("A"):
      v,r=tf.split(linear(h,z_dim*2),2,1) # splits into 2 parts on axis=1
...
saver = tf.train.Saver()  # keep all checkpoint files # max_to_keep=200

time_now = time.localtime()
string_time = time.strftime('%m.%d.%H.%M.%S', time_now)
directory_to_save='{}/{}/{}/'.format(os.getcwd(),'ltmp',string_time)
ckpt_file=directory_to_save

# summaries
tf.summary.scalar("loss", loss)
tf.summary.scalar("L_x", tf.reduce_mean(L_x))
tf.summary.scalar("L_x_next", tf.reduce_mean(L_x_next))
tf.summary.scalar("L_z", tf.reduce_mean(L_z))
...
if __name__=="__main__":
  ...
  sess.run(init)
  # WRITER
  writer = tf.summary.FileWriter('{}/graphs'.format(directory_to_save))

  dataset=PlaneData("data/plane2.npz","env_data/env1.png")
  dataset.initialize()
